...
#if 0
static int PM_USB_PostSuspend()
{
	return 0;
	}

static int PM_USB_PreResume()
{
	unsigned int tmp32;
	unsigned char tmp8;
...
static void PM_MAC_PostSuspend()
{
	int i = 0;
...
static void PM_I2C_PreResume(void)
{
	...
	/* Fix me */
	/*
	*(volatile unsigned long *)GPIO_ENABLE_CTL_2_ADDR &= ~(0x01000000|0x02000000);
	*(volatile unsigned long *)PMCEL_ADDR |= 0x0020;
	*/
#ifdef CONFIG_I2C_WMT
	/*Fix by Dean 2009/05/13*/
	GPIO_ENABLE_CTRL_11_VAL &= ~(0x00000003);
	PMCEL_VAL |= 0x0020;
	...
	*(volatile unsigned short *)(0xD8280000) = 0x0001;
	...
	GPIO_ENABLE_CTRL_11_VAL &= ~(0x00000006);
	PMCEL_VAL |= 0x0001;
