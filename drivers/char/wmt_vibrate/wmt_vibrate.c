...
struct vibrate_device {
	...
	// setting attributy
	unsigned int enablegpioreg;
	unsigned int enableoutputreg;
	unsigned int outputreg;
	unsigned int bitx;
}; 
...
static struct vibrate_device l_vibratedev = {
	...
	//default reg setting: SPI0SS as gpio
	.enablegpioreg = 0xD8110000+0x5C,
	.enableoutputreg = 0xD8110000+0x84,
	.outputreg = 0xD8110000+0xAC,
	.bitx = BIT3,
};
...
void wmt_do_vibrate(void)
{
	...
	// init gpio
	REG32_VAL(l_vibratedev.enablegpioreg) |= l_vibratedev.bitx; // 
	REG32_VAL(l_vibratedev.enableoutputreg) |= l_vibratedev.bitx; // gpio output enable
	// high level
	REG32_VAL(l_vibratedev.outputreg) |= l_vibratedev.bitx;
	// delay
...
{
	char buf[128];
	unsigned int ubootval[4];
	int ret = 0;

	// read the paraqm from
	memset(buf ,0, sizeof(buf));
	memset(ubootval, 0, sizeof(ubootval));
	if (wmt_getsyspara("vibratorset", buf, 127))
	{
		printk(KERN_ERR "Error when reading vibratorset from u-boot!!!\n");
		return -1;
	}
	dbg("vibrateor from uboot:%s****\n", buf);
	...
	if (ret != 4)
	{
		printk(KERN_ERR "Can't get vibrator setting from u-boot.\n");
		return -2;
	}
	l_vibratedev.enablegpioreg = ubootval[0];
	l_vibratedev.enableoutputreg = ubootval[1];
	l_vibratedev.outputreg = ubootval[2];
	l_vibratedev.bitx = 1<<ubootval[3];
	dbg("vibrator setting:%x,%x,%x,%x****\n", ubootval[0],
	                       ubootval[1],
	                       ubootval[2],
	                       ubootval[3]);
	return 0;
