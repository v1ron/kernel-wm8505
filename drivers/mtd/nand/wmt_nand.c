...
{
	struct WMT_nand_flash_dev *type = NULL;
	...
	*maf_id = chip->read_byte(mtd);
	...
	chip->chipsize = type->dwBlockCount * type->dwBlockSize;
	...
	*spec_clk = type->dwRWTimming;
	...
	if (!type->pagesize) {
		...
		if (*maf_id == 0xEC && dev_id == 0xd5 && chip->cellinfo == 0x94) {
	...
	if (*maf_id != NAND_MFR_SAMSUNG && !type->dwPageSize)
...
static int wmt_calc_clock(struct mtd_info *mtd, unsigned int spec_clk, int *T, int *divisor, int *Thold)
{
	...
	tADL = spec_clk&0xFF;
	for (i = 1; i < 16; i++) {
		if (56 >= ((PLLB&0x1F)*25)/i) {
			div1 = i;
			break;
		}
	}

	clk1 = (1000 * div1)/((PLLB&0x1F)*25);
	//printk("clk1=%d, div1=%d, spec_clk=%d\n", clk1, div1, spec_clk);
	for (T1 = 1; T1 < 10; T1++) {
		if ((T1*clk1) >= (tREA + 9)) /* 8.64 = tSKEW 5.64 + tDLY 2.42 + tSETUP 0.4 */
			break;
	}
	i = 1;
	while (i*clk1 <= tREH) {
	...
	*Thold = i;
	printk("T1=%d, clk1=%d, div1=%d, Thold=%d, tREA=%d+delay(9)\n", T1, clk1, div1, *Thold, tREA);
	Ttmp = *T = T1;
	clk = clk1;
	*divisor = div1;
	div2 = div1;
	while (Ttmp > 1 && clk != 0) {
		div2++;
		clk2 = (1000 * div2)/((PLLB&0x1F)*25);
		comp = 0;
		for (T2 = 1; T2 < Ttmp; T2++) {
			if ((T2*clk2) >= (tREA + 9)) { /* 8.64 = tSKEW 5.64 + tDLY 2.42 + tSETUP 0.4 */
				Ttmp = T2;
				comp = 1;
				i = 1;
				while (i*clk2 <= tREH) {
	...
	*Thold |= 0x10000;
	...
	if ((56 < ((PLLB&0x1F)*25)/(*divisor)) || clk == 0 || *T == 0 || clk > 45)
		return 1;

	return 0;
}

static void vt8500_nfc_init(struct vt8500_nand_info *info, struct mtd_info *mtd)
{
...
page_per_block = *(volatile unsigned long *)PMNAND_ADDR;
	...
	case 4:
		writel(0x1F, info->reg + VT8500_NFC_PAGESIZE_DIVIDER_SEL); /*32 page per block*/
	break;
	case 5:/*32 page per block*/
		writel((1 << 5) | 0x1F, info->reg + VT8500_NFC_PAGESIZE_DIVIDER_SEL);
	break;
	case 6:/*64 page per block*/
		writel((2 << 5) | 0x1F, info->reg + VT8500_NFC_PAGESIZE_DIVIDER_SEL);
	break;
	case 7:/*128 page per block*/
		writel((3 << 5) | 0x1F, info->reg + VT8500_NFC_PAGESIZE_DIVIDER_SEL);
	break;
	case 8:/*256 page per block*/
		writel((4 << 5) | 0x1F, info->reg + VT8500_NFC_PAGESIZE_DIVIDER_SEL);
	break;
	case 9:/*512 page per block*/
		writel((5 << 5) | 0x1F, info->reg + VT8500_NFC_PAGESIZE_DIVIDER_SEL);
	break;
	default:
	...
	NFC_RWTimming = ((Thold&0xFF) << 12) + ((T + (Thold&0xFF)) << 8) + (((Thold>>8)&0xFF) << 4) + 2*((Thold>>8)&0xFF);
...
static void set_ecc_engine(struct vt8500_nand_info *info, int type)
{
	...
	if (type == ECC8bit) {
		...
		writew(eccBCH_inetrrupt_enable, info->reg + VT8500_NFC_ECC_BCH_INT_MASK);
		...
		writew(eccBCH_inetrrupt_disable, info->reg + VT8500_NFC_ECC_BCH_INT_MASK);
		writel(readl(info->reg + VT8500_NFC_ECC_BCH_CTRL) | READ_RESUME,
		info->reg + VT8500_NFC_ECC_BCH_CTRL);
		#ifdef NAND_DEBUG
...
int column, unsigned int page_addr)
{
	...
	if (ecc_type == 1)
		writew(readw(info->reg + VT8500_NFC_ECC_BCH_CTRL) | READ_RESUME,
		info->reg + VT8500_NFC_ECC_BCH_CTRL);
	#endif
...
void bch4bit_data_ecc_correct(struct mtd_info *mtd)
{
	...
	if ((bank_stat1 & 0x101) == (ERR_CORRECT | BCH_ERR)) {
		...
		printk(KERN_NOTICE "in nfc_wait_idle(): Read data \n");
		#endif
		ecc_engine = readl(info->reg + VT8500_NFC_ECC_BCH_CTRL)&ECC_MODE;
		if (bch_ecc_idx > (ecc_engine*4)) {
		...
		for (i = 0; i < bch_ecc_idx; i++) {
			bch_err_pos[i] = (readw(info->reg +	VT8500_NFC_ECC_BCH_ERR_POS1 + 2*i) & 0x1fff);
			if((bch_err_pos[i] >> 3) < 512)
				bit_correct(&info->dmabuf[512 * bank + (bch_err_pos[i] >> 3)], bch_err_pos[i] & 0x07);

			#ifdef NAND_DEBUG
			printk(KERN_NOTICE "in nfc_wait_idle(): data area %xth ecc error position is byte%d bit%d\n",
			i, 512 * bank + (bch_err_pos[i] >> 3), (bch_err_pos[i] & 0x07));
			#endif
		}
...
void bch4bit_redunt_ecc_correct(struct mtd_info *mtd)
{
	...
	if ((bank_stat1 & 0x101) == (ERR_CORRECT | BCH_ERR)) {
		...
		#endif
		for (i = 0; i < bch_ecc_idx; i++) {
			bch_err_pos[i] = (readw(info->reg + VT8500_NFC_ECC_BCH_ERR_POS1 + 2*i) & 0x1ff);
...
int data_flag, int Nbank)
{
	...
	ReadDesc = (unsigned long *)(info->dmabuf + mtd->writesize + 0x100);
	WriteDesc = (unsigned long *)(info->dmabuf + mtd->writesize + 0x200);
	/*
	...
	*/
	...
	(wr) ? (unsigned long *)(info->dmaaddr + mtd->writesize + 0x200)
	: (unsigned long *)(info->dmaaddr + mtd->writesize + 0x100), wr);
...
static void vt8500_nand_cmdfunc(struct mtd_info *mtd, unsigned command, int column, int page_addr)
{
	struct vt8500_nand_info *info = vt8500_nand_mtd_toinfo(mtd);
	struct nand_chip *chip = mtd->priv;
	unsigned int addr_cycle = 0, b2r_stat;//, j;
	int status = -1;
...
read_again:
	#endif
	/* bug patch in 8bit ecc engine when read two page consectively with error*/
...
read_page_again:
			...
			if (ecc_type == 1) {
				...
				if (mtd->writesize == 2048) {
					...
					info->reg + VT8500_NFC_SMC_ENABLE);
							#else
					//printk(KERN_NOTICE " read oob--in vt8500_nand_cmdfunc(): config dma : NOT support read counter\n");
					vt8500_nfc_dma_cfg(mtd, 16, 0, -1, -1);
							#endif
	...
	if (mtd->writesize == 512) {
		...
		if (!(readw(info->reg + VT8500_NFC_SMC_ENABLE)&2)) {
			...
			printk(KERN_NOTICE "VT8500_NFC_SMC_ENABLE = 2\n");
		/* disable CE0 */
		...
		b2r_stat = readb(info->reg + VT8500_NFC_CHIP_ENABLE_CTRL);
		if ((b2r_stat&3) == 3) {
...
static int vt8500_nand_remove(struct platform_device *pdev)
{
...
static int vt8500_nand_probe(struct platform_device *pdev)
{
...
/* PM Support */
#ifdef CONFIG_PM
int vt8500_nand_suspend(struct platform_device *pdev, pm_message_t state)
{
	printk(KERN_NOTICE "vt8500_nand_suspend\n");
	return 0;
}

int vt8500_nand_resume(struct platform_device *pdev)
{
//	struct platform_device *pdev = to_platform(dev);
	struct vt8500_nand_info *info = dev_get_drvdata(&pdev->dev);
	struct vt8500_nand_mtd *nmtd;
...
//	return driver_register(&vt8500_nand_driver);
        return platform_driver_register(&vt8500_nand_driver);
}

static void __exit vt8500_nand_exit(void)
{
//	driver_unregister(&vt8500_nand_driver);
    platform_driver_unregister(&vt8500_nand_driver);
}
