...
int get_sf_info(struct wmt_flash_info_t *info)
{
    ...
    printk("Find SPI Flash, ID = 0x%08X\n", info->id);
...
int wmt_sfc_init(struct sfreg_t *sfc)
{
	...
	if (g_sf_info[0].phy&0xFFFF) {
		printk(KERN_ERR "VT8500 SFC Err : start address must align to 64KByte\n");
		return -1;
	}
	...
	if (g_sf_info[1].id != FLASH_UNKNOW) {
		...
		if (g_sf_info[0].phy&0xFFFF) {
			printk(KERN_ERR "VT8500 SFC Err : start address must align to 64KByte\n");
			printk(KERN_ERR "VT8500 SFC Err : CS1 could not be used\n");
			g_sf_info[1].id = FLASH_UNKNOW;
			return 0;
		}
...
{
	int ret;
	struct vt8500_sf_info_t *info = (struct vt8500_sf_info_t *)mtd->priv;
...
static int vt8500_sf_probe(struct platform_device *pdev)
{
	int err;
//	struct platform_device *pdev = to_platform_device(dev);
	struct vt8500_sf_info_t	*info;
	...
	if (err) {
		printk(KERN_ERR "vt8500 sf controller initial failed\n");
		goto exit_error;
	}
	printk(KERN_INFO "vt8500 sf controller initial ok\n");
...
static int vt8500_sf_remove(struct platform_device *pdev)
{
//	struct platform_device *pdev = to_platform_device(dev);
	struct vt8500_sf_info_t *info = dev_get_drvdata(&pdev->dev);
...
#ifdef CONFIG_PM
int vt8500_sf_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	printk(KERN_INFO "vt8500_sf_suspend\n");

	return 0;
}

int vt8500_sf_resume(struct platform_device *pdev)
{
//	struct platform_device *pdev = to_platform_device(dev);
	struct vt8500_sf_info_t *info = dev_get_drvdata(&pdev->dev);
	...
	if (info->reg)
		...
		printk(KERN_ERR "vt8500 sf restore state error\n");
...
#else
#define vt8500_sf_suspend NULL
#define vt8500_sf_resume NULL
#endif

/*
struct device_driver vt8500_sf_driver = {
		.name       = "sf",
		.bus          = &platform_bus_type,
		.probe       = vt8500_sf_probe,
		.remove     = vt8500_sf_remove,
		.suspend    = vt8500_sf_suspend,
		.resume     = vt8500_sf_resume
};
*/

static struct platform_driver vt8500_sf_driver = {
	.probe = vt8500_sf_probe,
	.remove = vt8500_sf_remove,
	.suspend        = vt8500_sf_suspend,
	.resume         = vt8500_sf_resume,
...
static int __init vt8500_sf_init(void)
{
	printk(KERN_INFO "VT8500 SPI Flash Driver, (c) 2007 VIA Electronics ltd.\n");
	//return driver_register(&vt8500_sf_driver);
	return platform_driver_register(&vt8500_sf_driver);
}

static void __exit vt8500_sf_exit(void)
{
	//driver_unregister(&vt8500_sf_driver);
	platform_driver_unregister(&vt8500_sf_driver);
}

/** need to init it early as other drivers may depends on it to get the configuration */
rootfs_initcall(vt8500_sf_init);
__exitcall(vt8500_sf_exit);

/*
module_init(vt8500_sf_init);
module_exit(vt8500_sf_exit);
*/

MODULE_AUTHOR("VIA MCE SW Team");
MODULE_DESCRIPTION("VT8500 SF driver");
MODULE_LICENSE("GPL");
