...
static unsigned int env_valid, env_invalid;
struct env_t {
...
static struct env_t *env_cache = NULL;
static int envcache_flag = 0; // 0 -- should first cp spi to ram, 1 -- Only find in cache.

static unsigned int env_crc32(unsigned char	*buf)
{
	unsigned int crc = 0;
	unsigned int len = ENV_MAX_SIZE - 5;
...
static unsigned int env_init_cache(void)
{
	int i;
	size_t retvarlen;
	unsigned char mtd_dev[20];
	unsigned char uenv[2];
	unsigned int chip_id, val, crc32, env;
	unsigned int bootflash = SPI_FLASH_TYPE;
	unsigned int env1 = 0xFF, env2 = 0xFF;
	...
	for (i = 0; i < MAX_MTD_DEVICES; i++) {
		if (!mtd_table[i])
			break;
		printk(KERN_DEBUG "MTD dev%d size: %8.8x \"%s\"\n",
	...
	for (i = 0; i < 2; i++) {
		...
		if (mtd_table[env]->read(
			...
			ENV_MAX_SIZE,
			&retvarlen,
			(unsigned char *)env_cache)) {
			printk(KERN_WARNING "## Warning: Read MTD Dev%d fail, retvarlen = %x\n",
				env, retvarlen);
			return 1;
		}
		if (env_cache->flags == 1) {
			crc32 = env_crc32(env_cache->data);
			if (env_cache->crc == crc32) {
				env_valid = env;
				if (env_valid == uenv[0])
					env_invalid = uenv[1];
				else
...
static unsigned char env_get_char_cache(int index)
{
	return env_cache->data[index];
}
...
static int envmatch_cache(unsigned char *s1, int i2)
{
	while (*s1 == env_get_char_cache(i2++))
		if (*s1++ == '=')
			return i2;
	if (*s1 == '\0' && env_get_char_cache(i2-1) == '=')
		return i2;
	return -1;
}
...
/*
 * Brief:
 *	Firstly get param from u-boot and stroe all u-boot param into cache,
 *	next get will read from the cache.
 * Input:
 *	varname -- the name of the enviromnet variable
 * Output:
 *	varval -- the value of the enviromnet variable
 * Return:
 ...
 */
int wmt_getsyspara_cache(char *varname, unsigned char *varval, int varlen)
{
	int i, j, k, nxt;
	int rcode = 0;
	...
	if (!envcache_flag)
	{
		// First cp uboot arg into cache from spi
		if (env_init_cache()) 
		{
			rcode++;
			goto out;
	...
	k = -1;
	i = 0;
	for (j = 0; env_get_char_cache(j) != '\0'; j = nxt+1) {

		for (nxt = j; env_get_char_cache(nxt) != '\0'; ++nxt)
			;
		k = envmatch_cache((unsigned char *)varname, j);
		if (k < 0)
			continue;
		while (k < nxt)
			varval[i++] = env_get_char_cache(k++);
		break;
	}
	varval[i] = '\0';
...
EXPORT_SYMBOL_GPL(wmt_getsyspara_cache);
...
int wmt_setsyspara(char *varname, char *varval)
{
	...
	// If first, cp all env variable of u-boot to the cahe
	// Find the needed env variable of u-boot
...
{
    // alloc the cache of u-boot env
    env_cache = kmalloc(ENV_MAX_SIZE, GFP_KERNEL);
    if (!env_cache)
    {
    	printk(KERN_ERR "Can't alloc the cache of u-boot env!!!\n");
    	return -ENOMEM;
    }
...
{
    // release the cache of u-boot env
    if (env_cache)
    {
    	kfree(env_cache);
    }
...
module_init(ubootenv_init);
module_exit(ubootenv_exit);
MODULE_LICENSE("GPL");
