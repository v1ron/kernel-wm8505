...
extern  void serio_cleanup(struct serio *serio);
...
/*
module_param(i8042_noaux, bool, 0444);
MODULE_PARM_DESC(i8042_noaux, "Do not probe or use AUX (mouse) port.");

module_param(i8042_nomux, bool, 0444);
MODULE_PARM_DESC(i8042_nomux, "Do not check whether an active multiplexing controller is present.");

module_param(i8042_unlock, bool, 0444);
MODULE_PARM_DESC(i8042_unlock, "Ignore keyboard lock.");

module_param(i8042_reset, bool, 0444);
MODULE_PARM_DESC(i8042_reset, "Reset controller during init and cleanup.");

module_param(i8042_direct, bool, 0444);
MODULE_PARM_DESC(i8042_direct, "Put keyboard port into non-translated mode.");

module_param(i8042_dumbkbd, bool, 0444);
MODULE_PARM_DESC(i8042_dumbkbd, "Pretend that controller can only read data from keyboard");

module_param(i8042_noloop, bool, 0444);
MODULE_PARM_DESC(i8042_noloop, "Disable the AUX Loopback command while probing for the AUX port");
*/
...
static int i8042_open(struct serio *port)
{
	...
	if (request_irq(values->irq, i8042_interrupt,
			IRQF_SHARED, "i8042", i8042_platform_device)) {
 ...
 */
		...
		if (likely(i8042_mux_values[aux_idx].exists))
			serio_interrupt(i8042_mux_port[aux_idx], data, dfl);
 ...
 */
	...
	if (request_irq(values->irq, i8042_interrupt, IRQF_SHARED,
...
static int i8042_controller_init(void)
{
	...
	/*Fix bugs mouse timeout issue Added by Eric*/
	(*(volatile int *)(__KBDC_BASE+0x10)) |= 0x0100;
#endif
/*
 ...
 */
	...
	if (1) {
		...
		printk("I8042_CMD_CTL_TEST ok 0x%x\n",param);
 ...
 */
static int i8042_suspend(struct platform_device *pdev,  pm_message_t state)
{
	return i8042_controller_suspend();
	//return level == SUSPEND_DISABLE ? i8042_controller_suspend() : 0;
}

static int i8042_resume(struct platform_device *pdev)
{
	 return i8042_controller_resume();
	//return level == RESUME_ENABLE ? i8042_controller_resume() : 0;
}

static void i8042_shutdown(struct platform_device *pdev)
{
	i8042_controller_cleanup();
}
/*
...
int __init i8042_init(void)
{
	...
	i8042_aux_values.irq = IRQ_MOUSE; //I8042_AUX_IRQ;
	i8042_kbd_values.irq = IRQ_KEYBOARD; //I8042_KBD_IRQ;
