...
)
{
	...
	for (i = 0; ret >= 0 && i < num; i++) {
		...
		if (pmsg->flags & I2C_M_RD)/* READ*/
			ret = adap->read_msg(pmsg->addr , pmsg->buf, pmsg->len, restart, last) ;
		else/* Write*/
			ret = adap->write_msg(pmsg->addr , pmsg->buf, pmsg->len, restart, last) ;
	}
...
struct i2c_algorithm wmt_i2c_algorithm  = {
	name:                   "WMT I2C Algorithm",
	id:                     I2C_ALGO_WMT,
	master_xfer:            wmt_i2c_xfer,
	smbus_xfer:             NULL,
	slave_send:             NULL,
	slave_recv:             NULL,
	algo_control:           wmt_i2c_control,
