/*++
	...
	Abstract:
		This program is a RTC driver for WMT based EVB.

	Revision History:
		Jan.31.2005 First created
		Jun.17.2005 Revsion 0.53, refined the whole source code only.
                Jan.08.2010 Revision 0.54, fixed it for Android SDK 1.6 (Linux 2.6.29) by howayhuo
...
static int wmt_rtc_ioctl(struct device *dev, unsigned int cmd, unsigned long arg)
{
	...
	*/
	 ...
	 */
		...
		return 0;
    case RTC_RD_TIME:
        return wmt_rtc_read_time(dev, (struct rtc_time *)arg);
    case RTC_SET_TIME:
        return wmt_rtc_set_time(dev, (struct rtc_time *)arg);
...
static int wmt_rtc_probe(struct platform_device *pdev)
{
    ...
    if ((rtc_on) && ((*(volatile unsigned int *)0xd8120000) > 0x34260102)) 
    {
        printk("wmt_rtc on\n");
...
#else
#define wmt_rtc_suspend	NULL   //sa1100_rtc_suspend
#define wmt_rtc_resume	NULL   //sa1100_rtc_resume
#endif
