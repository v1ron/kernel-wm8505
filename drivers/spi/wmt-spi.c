...
#define SPI1_CLK_DIVISOR_VAL  0x06
#define SPI2_CLK_DIVISOR_VAL  0x06

#define SPI0_CLK_LOW_DIVISOR_VAL  0
#define SPI1_CLK_LOW_DIVISOR_VAL  0
#define SPI2_CLK_LOW_DIVISOR_VAL  0
...
static int spi_dev_major = 154; //153;
static int spi_dev_minor = 0;
...
)
{
	...
	*/
	...
	if (port == 0)
		...
		spi_base_addr = SPI1_REG_BASE;
	else
		spi_base_addr = SPI2_REG_BASE;
	...
	/* Initial IRQ*/
	if (port == 0)
		spi_port->irq_num = IRQ_SPI0;	/* Setting SPI IRQ Number*/
	else if (port == 1)
		spi_port->irq_num = IRQ_SPI1;	/* Setting SPI IRQ Number*/
	else
		spi_port->irq_num = IRQ_SPI2;	/* Setting SPI IRQ Number*/
	/*********************** DMA RX *************************/
	/* Initial read in dma*/
	...
	if (port == 0)
		...
		spi_port->rdma.config.DeviceReqType = SPI1_DMA_RX_REQ;
	else
		spi_port->rdma.config.DeviceReqType = SPI2_DMA_RX_REQ;
	...
	if (port == 0)
		...
		spi_port->wdma.config.DeviceReqType = SPI1_DMA_TX_REQ;
	else
		spi_port->wdma.config.DeviceReqType = SPI2_DMA_TX_REQ;
...
)
{
	...
	else if (spi_port->port == 1)
		REG8_VAL(SPI1_CLOCK_DIVISOR)  = SPI1_CLK_DIVISOR_VAL;
	else if (spi_port->port == 2)
		REG8_VAL(SPI2_CLOCK_DIVISOR)  = SPI2_CLK_DIVISOR_VAL;
	/* check user valid*/
	if (!spi_user) {
		printk(KERN_ERR "[SPI]: null user enter %s.\n", __func__);
		return;
	...
	if (divisor > 0x7FF) {
		if (spi_port->port == 0)
			...
			REG8_VAL(SPI1_CLOCK_DIVISOR)  = SPI1_CLK_LOW_DIVISOR_VAL;
		else if (spi_port->port == 2)
			REG8_VAL(SPI2_CLOCK_DIVISOR)  = SPI2_CLK_LOW_DIVISOR_VAL;
		if (spi_user->freq != 0)
...
static unsigned int get_spi_input_freq(int port)
{
	...
	unsigned char pmc_pll_b_mutiplier;
	unsigned int pmc_pll_b_freq;
	unsigned char pmc_spi_divisor;

	/*PMC 204 - PLL_B  Multiplier and Range Values Register*/
	unsigned int volatile *pmc_pll_b = (unsigned int *)(0xD8130000 + 0x204);
	/*
	 * PMC 33C - SPI0 Clock Divisor Value Register
	 * PMC 340 - SPI1 Clock Divisor Value Register
	 * PMC 344 - SPI2 Clock Divisor Value Register
	 */
	unsigned char volatile *pmc_spi0_divisor = (unsigned char *)(0xD8130000 + 0x33c);
	unsigned char volatile *pmc_spi1_divisor = (unsigned char *)(0xD8130000 + 0x340);
	unsigned char volatile *pmc_spi2_divisor = (unsigned char *)(0xD8130000 + 0x344);

	if ((*pmc_pll_b & 0x1f) < 4)
		pmc_pll_b_mutiplier = 1;
	else
		pmc_pll_b_mutiplier = (*pmc_pll_b & 0x1f) << 1;

	pmc_pll_b_freq = PLL_25MHZ * pmc_pll_b_mutiplier;

	if (port == 0)
		pmc_spi_divisor = (*pmc_spi0_divisor & 0x1f);
	else if (port == 1)
		pmc_spi_divisor = (*pmc_spi1_divisor & 0x1f);
	else
		pmc_spi_divisor = (*pmc_spi2_divisor & 0x1f);

	if (pmc_spi_divisor == 0)
		return pmc_pll_b_freq / 2 / 32;
	else
		return pmc_pll_b_freq / 2 / pmc_spi_divisor;
...
)
{
	...
	*/
	...
	if ((spi_user->op_mode == SPI_DMA_MODE) && (spi_user->dma_init)) {
		...
		DPRINTK("[spi_write_data] spi_port->wdma.dmach = %d\n",
			spi_port->wdma.dmach);
		DPRINTK("[spi_write_data] spi_port->wdma.config.DeviceReqType = %d\n",
			spi_port->wdma.config.DeviceReqType);
		DPRINTK("[spi_write_data] spi_port->wdma.config.DefaultCCR=0x%08x\n",
			spi_port->wdma.config.DefaultCCR);		
		DPRINTK("[spi_write_data] spi_port->wdma.config.ChunkSize = %d\n",
			spi_port->wdma.config.ChunkSize); /*added by howay*/
...
)
{
	...
	if (portIndex == 0)
		REG32_VAL(GPIO_BASE_ADDR + 0x5C) &= ~(BIT0 | BIT1 | BIT2 | BIT3);
	else if (portIndex == 1)
		REG32_VAL(GPIO_BASE_ADDR + 0x5C) &= ~(BIT4 | BIT5 | BIT6 | BIT7);
	else
		REG32_VAL(GPIO_BASE_ADDR + 0x5C) &= ~(BIT8 | BIT9 | BIT10 | BIT11);
...
)
{
	...
	case SPIIOSET_FREQ: /* Set bus frequency for the user*/
		...
		/*--> modified by howay 20070913*/
		/*
		if (value < AHB_FREQ)	// Check boundary
	...
	case SPIIO_WRITE:	/* Write out data*/
		...
		break;
#if 0
	case SPIIO_READANDWRITE:	/* Read in data before write out data*/
		/* Get transfer requirment from user space*/
		retval = copy_from_user((struct spi_xfer_req_s *)&spi_xfer_req,
								(const void *)arg,
								sizeof(struct spi_xfer_req_s));

		if (spi_xfer_req.size > 0x20)
			return -EINVAL;

		rbuf = (unsigned char *)kmalloc(spi_xfer_req.size, GFP_KERNEL);	/* allocate buffer*/

		if (rbuf == NULL) /* there is no memory for saving read-in data*/
			return -EBUSY;

		wbuf = (unsigned char *)kmalloc(spi_xfer_req.size, GFP_KERNEL);	/* allocate buffer*/
		...
		copy_from_user(wbuf, spi_xfer_req.wbuf, spi_xfer_req.size);

		/* read in data then write out data*/
		spi_read_and_write(spi_user, spi_xfer_req.wbuf, rbuf, spi_xfer_req.size);

		copy_to_user(spi_xfer_req.rbuf, rbuf, spi_xfer_req.size);
...
* \retval  0 if success
*/
static int spi_probe(struct platform_device* platdev)
{
	int port;
	int ret = 0;
	dev_t dev_no;
	...
	for (port = 0; port < SPI_PORT_NUM; port++) {
		...
		/* Request IRQ*/
		//if (request_irq(SPI_PORT[port].irq_num, spi_isr, SA_SHIRQ, "SPI-IRQ", &SPI_PORT[port]) < 0) {
...
* \retval  0 if success
*/
static int spi_remove(struct platform_device* platdev)
{
	//struct cdev *cdev;
	int port;
...
* \retval  0 if success
*/
static int spi_suspend(struct platform_device* platdev, 
pm_message_t state)
{
...
* \retval  0 if success
*/
static int spi_resume(struct platform_device* platdev)
{
...
static struct platform_driver spi_driver = {
	//.name           = "wmt_spi", /* This name should equal to platform device name.*/
...
* \retval  0 if success
*/
static struct class* l_dev_class;
static struct device *l_device;
static int spi_init(void)
{
	...
	// firstly we should creat wmt_spi node
	// register char device
	if (register_chrdev (spi_dev_major, "wmt_spi", &spi_fops)) {
		printk (KERN_ERR "%s: unable to get major %d\n", __func__, spi_dev_major);
		error = -EIO;
		return error;
	}
	l_dev_class = class_create(THIS_MODULE, "wmt_spi");
	if (IS_ERR(l_dev_class))
	{
		error = PTR_ERR(l_dev_class);
		printk(KERN_ERR "Can't class_create  wmt_spi device !!\n");
		//goto initend1;
		return error;
	}
	l_device = device_create(l_dev_class, NULL, MKDEV(spi_dev_major, 0), NULL, "wmt_spi");
    if (IS_ERR(l_device))
    {
    	error = PTR_ERR(l_device);
    	printk(KERN_ERR "Failed to create device %s !!!", "wmt_spi");
    	//goto initend2;
    	return error;
    }
	/*
...
static void spi_exit(void)
{
	...
	device_destroy(l_dev_class, MKDEV(spi_dev_major, 0));
    class_destroy(l_dev_class);
	unregister_chrdev(spi_dev_major, "wmt_spi");
...
EXPORT_SYMBOL(set_callback_func);
