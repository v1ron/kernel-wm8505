...
/*
 * Revision History
 * HP01 - 05/29/2008 - Support MJPEG compression quality
 */
/* ------------------------------------------------------------------------
 * UVC Controls
 */
...
//HP01_begin
int uvc_probe_video(struct uvc_video_device *video, struct uvc_streaming_control *probe, __u32 quality)
//HP01_end
{
		...
		if ((ret = uvc_get_video_ctrl(video, &probe_min, 1, GET_MIN)) < 0 ||
			...
			case HIGH_QUALITY:
				probe->wCompQuality = probe_max.wCompQuality;
			     break;
                        case MEDIUM_QUALITY:
			        probe->wCompQuality = (probe_max.wCompQuality+probe_min.wCompQuality)>>1;
			     break;		
			case LOW_QUALITY:
		...
		struct uvc_buffer *buf, const __u8 *data, int len)
{
	...
	if (data[1] & UVC_STREAM_ERR) {
		uvc_trace(UVC_TRACE_FRAME, "Dropping payload (error bit set).\n");
		return -ENODATA;
	}
...
static int uvc_init_video_isoc(struct uvc_video_device *video, struct usb_host_endpoint *ep)
{
	...
	psize = (psize & 0x07ff) * (1 + ((psize >> 11) & 3));
        ...
        //hpchen - 03/19/2008 - patch for 3400 A0 4k alignment problem.
	/*if ((0x1000%psize) !=0)
            {
		psize = ((psize+0x3FF)/0x400)*0x400;
              }*/
