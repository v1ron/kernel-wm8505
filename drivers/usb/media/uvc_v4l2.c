...
/*
 * Revision History:
 * HP01 - 05/29/2008 - Support for MJPEG compression quality
 */
/* ------------------------------------------------------------------------
 * V4L2 interface
 */
		...
		struct uvc_frame **uvc_frame)
{
	...
	/* Probe the device */
//HP01_Begin
	if ((ret = uvc_probe_video(video, probe,fmt->fmt.pix.priv)) < 0)
//HP01_End
		goto done;
...
//HP01_begin
	if ((ret = uvc_probe_video(video, &probe, parm->parm.capture.reserved[0])) < 0)
//HP01_end
		return ret;
