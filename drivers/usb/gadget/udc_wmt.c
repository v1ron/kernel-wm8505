 ...
 */
 ...
 /* Beta-0.90 2007.12.06 Giga Hsu : PASS Chapter 9 Compliance Test.*/
 /* Beta-0.91 2008.01.22 Giga Hsu : (1) PASS MSC(Mass Storage Class) Compliance Test.*/
 /*                                 (2) Enable RNDIS path.*/
...
/*static struct GLOBAL_REGISTER *global_reg;*/
...
wmt_alloc_request(struct usb_ep *_ep, int gfp_flags)
{
	struct vt8500_req	*req;
	struct vt8500_ep	*ep = NULL;

	DBG("wmt_alloc_request() %s\n", ep->name);
...
static void next_in_dma(struct vt8500_ep *ep, struct vt8500_req *req)
{
	...
	if (((ep->bEndpointAddress & 0x7F) == 1) && (req->req.dma == 0xFFFFFFFF) && (ep->rndis == 0)) {
		...
		retval = dma_alloc_coherent(ep->udc->dev,
							req->req.length, &dma, GFP_KERNEL|GFP_ATOMIC);
...
#ifdef RNDIS_INFO_DEBUG_BULK_IN
	if ((ep->bEndpointAddress & 0x7F) == 1)
		INFO("finish_in_dma() %s req->req.actual(0x%08X) req->req.length(0x%08X)\n",
...
#ifdef RNDIS_INFO_DEBUG_BULK_OUT
	if (ep->bEndpointAddress == 2)
		INFO("finish_out_dma() %s req->req.actual(0x%08X) req->req.length(0x%08X)\n",
			ep ? ep->ep.name : NULL, req->req.actual, req->req.length);
#endif
...
static int
wmt_ep_queue(struct usb_ep *_ep, struct usb_request *_req, int gfp_flags)
{
	struct vt8500_ep	*ep = container_of(_ep, struct vt8500_ep, ep);
	struct vt8500_req	*req = container_of(_req, struct vt8500_req, req);
	/*struct vt8500_udc	*udc;*/
	...
	if ((ep->bEndpointAddress == 2))/* || ((ep->bEndpointAddress & 0x7F) == 1))*/
		...
		ep->ep.name, _req, _req->req.length, _req->buf);
#endif
...
static void udc_control_prepare_data_resp(void)/*(struct vt8500_udc *udc, u32 udc_irq_src)*/
{
	...
	/* SETUP starts all control transfers*/
	{
		...
		case USB_REQ_SET_CONFIGURATION:
			...
			printk("set conf %x %x\n",u.r.bRequestType,u.r.wLength);
			if (u.r.bRequestType != USB_RECIP_DEVICE)/*USB_RECIP_DEVICE(0x00)*/
				{printk("setconf5\n");
				goto delegate;

				}
			if (u.r.wLength != 0){printk("setconf6\n");
				goto do_stall;}
			udc->ep0_set_config = 1;
			udc->ep0_reset_config = (u.r.wValue == 0);
			printk("set config %d\n", u.r.wValue);
			printk("udc->ep0_reset_config %d\n",udc->ep0_reset_config);
...
static void wmt_pdma0_reset(void)
{
	unsigned int pdma1_int_temp32 = 0;
	unsigned int DMA_Descriptor_Point1_temp32 = 0;
	unsigned int DMA_Residual_Bytes1_temp32 = 0;
	unsigned int DMA_Data_Addr1_temp32 = 0;
	unsigned int DMA_Branch_Addr1_temp32 = 0;
	unsigned int Descriptor_Addr1_temp32 = 0;
	unsigned int DMA_Context_Control1_temp32 = 0;

	pdma1_int_temp32 = pUdcDmaReg->DMA_IER & DMA_INTERRUPT_ENABLE1;
	DMA_Descriptor_Point1_temp32 = pUdcDmaReg->DMA_Descriptor_Point1;
	DMA_Residual_Bytes1_temp32 = pUdcDmaReg->DMA_Residual_Bytes1;
	DMA_Data_Addr1_temp32 = pUdcDmaReg->DMA_Data_Addr1;
	DMA_Branch_Addr1_temp32 = pUdcDmaReg->DMA_Branch_Addr1;
	Descriptor_Addr1_temp32 = pUdcDmaReg->Descriptor_Addr1;
	DMA_Context_Control1_temp32 = pUdcDmaReg->DMA_Context_Control1;
	...
	pUdcDmaReg->DMA_IER |= pdma1_int_temp32;
	pUdcDmaReg->DMA_Descriptor_Point1 = DMA_Descriptor_Point1_temp32;
	pUdcDmaReg->DMA_Residual_Bytes1 = DMA_Residual_Bytes1_temp32;
	pUdcDmaReg->DMA_Data_Addr1 = DMA_Data_Addr1_temp32;
	pUdcDmaReg->DMA_Branch_Addr1 = DMA_Branch_Addr1_temp32;
	pUdcDmaReg->Descriptor_Addr1 = Descriptor_Addr1_temp32;
	pUdcDmaReg->DMA_Context_Control1 = DMA_Context_Control1_temp32;
...
static void wmt_pdma_init(struct device *dev)
{
	...
	UdcPdmaVirAddrLI = dma_alloc_coherent(pDMADescLI, 0x100, &UdcPdmaPhyAddrLI, GFP_KERNEL|GFP_ATOMIC);
	UdcPdmaVirAddrSI = dma_alloc_coherent(pDMADescSI, 0x100, &UdcPdmaPhyAddrSI, GFP_KERNEL|GFP_ATOMIC);
	UdcPdmaVirAddrLO = dma_alloc_coherent(pDMADescLO, 0x100, &UdcPdmaPhyAddrLO, GFP_KERNEL|GFP_ATOMIC);
	UdcPdmaVirAddrSO = dma_alloc_coherent(pDMADescSO, 0x100, &UdcPdmaPhyAddrSO, GFP_KERNEL|GFP_ATOMIC);
...
static int __devinit wmt_udc_probe(struct platform_device *odev)
{
...
static int __devexit wmt_udc_remove(struct platform_device *odev)
{
	/*struct platform_device	*odev = to_platform_device(dev);*/
...
static int wmt_udc_suspend(struct platform_device *odev, pm_message_t state)
{
	printk("wmt_udc_suspend()\n");
//--> added by howayhuo on 20100429
//When the USB Device is being writed, we don't permit UDC suspend.
//Otherwise it destroy the USB Device.
//But we don't know how to judge the USB Device is being writed
//so we judge whether the USB Device is connected
//If the USB Device is connected, we don't permit UDC suspend
        if(pDevReg->SelfPowerConnect & 0x01)
        {
            printk("USB Device is connected, don't permit suspend\n");
	    return -1;
        }
//<--
	pDevReg->CommandStatus &= 0x1F;
...
/*	if ((state == 3) && (level == 3))*/
/*		*(volatile unsigned int *)0xd8130254 &= ~0x00000080;*/   /*DPM needed*/
	if((*(volatile unsigned short*)0xd8120002) == 0x3426)
		if((*(volatile unsigned short*)0xd8120000) >= 0x0103)
			*(volatile unsigned char*)0xd8007249 |= 0x04;
	return 0;

} /*wmt_udc_suspend()*/

static int wmt_udc_resume(struct platform_device *odev)
{
	printk("wmt_udc_resume()\n");
/*	if (!((*(volatile unsigned int *)0xd8130254)&0x00000080))*/
/*		*(volatile unsigned int *)0xd8130254 |= 0x00000080;*/      /*DPM needed*/
	if((*(volatile unsigned short*)0xd8120002) == 0x3426)
		if((*(volatile unsigned short*)0xd8120000) >= 0x0103)
			*(volatile unsigned char*)0xd8007249 &= ~0x04;
...
static struct platform_driver udc_driver = {
	...
	.probe		= wmt_udc_probe,
	.remove		= __devexit_p(wmt_udc_remove),
	.suspend	= wmt_udc_suspend,
	.resume		= wmt_udc_resume,
};
