...
** $Log: POST/vpp/govm.c $
 *
 * 01	08/05/15, Andy Chiu, Initial version
...
#ifdef GOVM_DEBUG
#define GOVMMSG(fmt, args...)  DPRINT("[GOVM] %s: " fmt, __FUNCTION__ , ## args)
#else
#define GOVMMSG(fmt, args...) do {} while(0)
#endif
...
void govm_set_disp_coordinate(U32 x, U32 y)
{
	...
	REG32_VAL(0xd8050650) = x-1;
	REG32_VAL(0xd8050654) = y-1;
	REG32_VAL(0xd8050634) = x-1;
	REG32_VAL(0xd805063c) = y-1;
}
...
void govm_set_pip_color_format(vdo_color_fmt format)
{
	...
	if( format == VDO_COL_FMT_ARGB ){
		if( govw_get_hd_color_format() != VDO_COL_FMT_ARGB ){
			GOVMMSG("*E* govw & pip color fmt not match");
		}
	}
	else {
		if( govw_get_hd_color_format() == VDO_COL_FMT_ARGB ){
...
{
	GOVMMSG("y_addr:0x%08x, c_addr:0x%08x\n",y_addr,c_addr);
...
void govm_suspend(int sts)
{
	switch( sts ){
		...
		case 2:	// backup register
			p_govm->reg_bk = vpp_backup_reg(GOVM_BASE_ADDR+0x00,0xEC); /* 0x00 - 0xEC */
			break;
		default:
			break;
	}
...
void govm_resume(int sts)
{
	switch( sts ){
		case 0:	// restore register
			vpp_restore_reg(GOVM_BASE_ADDR+0x00,0xEC,p_govm->reg_bk); /* 0x00 - 0xEC */	
			p_govm->reg_bk = 0;			
			break;
