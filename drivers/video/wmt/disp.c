...
/*--- History ------------------------------------------------------------------- 
* Version 0.01 , Sam Shen, 2009/03/26
*           First version
*
*------------------------------------------------------------------------------*/
...
// #define DEBUG
#ifdef DEBUG
#define DBGMSG(fmt, args...)  DPRINT("[DISP] %s: " fmt, __FUNCTION__ , ## args)
#else
...
void disp_reg_dump(void)
{
	...
	for(i=0;i<0xFC;i+=16){
		DPRINT("0x%08x : 0x%08x 0x%08x 0x%08x 0x%08x\n",DISP_BASE_ADDR+i,vppif_reg32_in(DISP_BASE_ADDR+i),
			vppif_reg32_in(DISP_BASE_ADDR+i+4),vppif_reg32_in(DISP_BASE_ADDR+i+8),vppif_reg32_in(DISP_BASE_ADDR+i+12));
	}

	for(i=0x0;i<0x1C;i+=16){
		DPRINT("0x%08x : 0x%08x 0x%08x 0x%08x 0x%08x\n",DISP_BASE2_ADDR+i,vppif_reg32_in(DISP_BASE2_ADDR+i),
			vppif_reg32_in(DISP_BASE2_ADDR+i+4),vppif_reg32_in(DISP_BASE2_ADDR+i+8),vppif_reg32_in(DISP_BASE2_ADDR+i+12));
	}
	...
	DPRINT("COLORBAR %d, Dot Crawl %d,Clamp %d,inv clken %d\n",vppif_reg32_read(DISP_COLORBAR_EN),vppif_reg32_read(DISP_DOT_CRAWL),
		vppif_reg32_read(DISP_CLAMP_EN),vppif_reg32_read(DISP_INVTVCLK_EN));
...
void disp_set_enable(vpp_flag_t enable)
{
	...
	if( enable  || 	vppif_reg32_read(DISP_EXTTV_EN)){
		...
		if( vppif_reg32_read(DISP_EN) ){
			...
			vpp_proc_flag(VPP_INT_DISP_VBIE,&p_disp->disable_flag,1);
...
void disp_patch(vpp_tvsys_t tvsys,vpp_tvconn_t tvconn)
{
	if( tvsys == VPP_TVSYS_NTSC ){
		switch( tvconn ){
			case VPP_TVCONN_CVBS:
			case VPP_TVCONN_SVIDEO:
				vppif_reg32_out(0xd805071c,0x0252506c);
				vppif_reg32_out(0xd8050718,0x3);
				vppif_reg32_out(0xd8050730,0x3);
				vppif_reg32_out(0xd8050708,0x01c70c06);
				vppif_reg32_out(0xd805072c,0x0);
				vppif_reg32_out(0xd8050724,0x16);
				vppif_reg32_out(0xd8050744,0x3e);
				vppif_reg32_out(0xd805073c,0x187f);
				vppif_reg32_out(0xd8050728,0x00300079);
				break;
			case VPP_TVCONN_YCBCR:
				vppif_reg32_out(0xd8050720,0x007a8888);
				vppif_reg32_out(0xd8050728,0x00300078);
				break;
			case VPP_TVCONN_YPBPR:
				vppif_reg32_out(0xd80507a0,0x027a8080);
				vppif_reg32_out(0xd80507a4,0x10017100);
				break;
			default:
				break;
		}
	...
	if( tvsys == VPP_TVSYS_PAL ){
		switch( tvconn ){
			...
			case VPP_TVCONN_SVIDEO:
				vppif_reg32_out(0xd8050708,0x01c70c06);
				vppif_reg32_out(0xd805071c,0x02555373);
				vppif_reg32_out(0xd805072c,0x0);
				vppif_reg32_out(0xd8050728,0x0030007a);
				vppif_reg32_out(0xd8050744,0x3e);
				break;
			case VPP_TVCONN_YCBCR:
				vppif_reg32_out(0xd8050720,0x008a8e8e);
				vppif_reg32_out(0xd8050728,0x00300075);
				break;
			case VPP_TVCONN_YPBPR:
				vppif_reg32_out(0xd80507a0,0x02808080);
				vppif_reg32_out(0xd80507a4,0x01017100);
				break;
			default:
				break;
		}
...
void disp_set_mode(vpp_tvsys_t tvsys,vpp_tvconn_t tvconn)
{
	...
	vpp_set_base_clock(VPP_MOD_GOVRH, 27000000);
	...
	// Timing
	p_tim = disp_get_timing(p_attr->VOS,interlace,vppif_reg32_read(DISP_EXTTV_EN));
...
void disp_suspend(int sts)
{
	switch( sts ){
		...
		case 2:	// backup register
			p_disp->reg_bk = vpp_backup_reg(DISP_BASE_ADDR+0x00,0xFC); /* 0x00 - 0xFC */
			disp_reg_bk2 = vpp_backup_reg(DISP_BASE2_ADDR+0x00,0x1C); /* 0x00 - 0x1C */
			break;
		default:
			break;
	}
...
void disp_resume(int sts)
{
	switch( sts ){
		case 0:	// restore register
			vpp_restore_reg(DISP_BASE_ADDR+0x00,0xFC,p_disp->reg_bk); /* 0x00 - 0xFC */
			vpp_restore_reg(DISP_BASE2_ADDR+0x00,0x1C,disp_reg_bk2); /* 0x00 - 0x1C */
			disp_reg_bk2 = 0;
			p_disp->reg_bk = 0;			
			break;
