...
** $Log: POST/vpp/scl.c $
 *
 * 01	08/10/08, Andy Chiu, Initial version
 *
 */
#define SCL_C
#include "scl.h"
...
#ifdef WMT_FTBLK_SCL
#define REG_BASE_SCL444U 	SCL_BASE_ADDR

#ifdef SCL_DEBUG
#define SCLMSG(fmt, args...) DPRINT("[SCL] %s: " fmt, __FUNCTION__ , ## args)
#else
#define SCLMSG(fmt, args...) do {} while(0)
#endif
...
void scl_reg_dump(void)
{
	...
	DPRINT("clk %d,Read cyc %d\n",vpp_get_base_clock(VPP_PLL_B,VPP_DIV_VPP),vppif_reg32_read(SCL_TG_RDCYC));
...
//	SCLMSG("vpu_set_V_scale(%d,%d)\r\n", A, B);
	...
	V_THR_DIV2 = A;
//	SCLMSG("V step %d,sub step %d, div2 %d\r\n", V_STEP, V_SUB_STEP, V_THR_DIV2);
...
//	SCLMSG("vpu_set_H_scale(%d,%d)\r\n", A, B);
	...
	H_THR_DIV2 = A;
//	SCLMSG("H step %d,sub step %d, div2 %d\r\n", H_STEP, H_SUB_STEP, H_THR_DIV2);
...
void scl_set_timing(vpp_clock_t *timing)
{
...
void sclr_set_fb_addr(U32 y_addr, U32 c_addr)
{
	...
	SCLMSG("y_addr:0x%08x, c_addr:0x%08x\n",y_addr,c_addr);

	vppif_reg32_out(REG_SCLR_YSA,y_addr);
	vppif_reg32_out(REG_SCLR_CSA,c_addr);
	line_offset = vppif_reg32_read(SCLR_YBUFWID);
#ifdef REG_SCLR_YSA2	
	vppif_reg32_out(REG_SCLR_YSA2,y_addr+line_offset);
	switch(sclr_get_color_format()){
		case VDO_COL_FMT_YUV420:
			line_offset /= 2;
			break;
	...
	vppif_reg32_out(REG_SCLR_CSA2,c_addr+line_offset);
#endif	
}
...
// #if 0
#ifdef PATCH_VPP_SCALE_DOWN_QUALITY	// patch for special case : 3/4, 4/5, 5/6 ...
	if(((A+1) == B) && (B>3)){
		index = 0;
		
		/* 1st bypass */
		scl_set_H_rec_table(index,0);	// bypass
		scl_set_H_scale_table(index,1);
		scl_set_H_div_table(index,0);
		index++;

		/* 2nd store into buffer */
		scl_set_H_rec_table(index,0);
		scl_set_H_scale_table(index,0);
		scl_set_H_div_table(index,0);
		index++;

		/* other : interpolation with pre pixel */
		for(i=0;i<(B-2);i++){
			scl_set_H_rec_table(index,0);
			scl_set_H_scale_table(index,3);
			scl_set_H_div_table(index,0x80);
			index++;
		}
		return;
	}
...
// #if 0
	...
	if(((A+1) == B) && (B>3)){
		...
		/* 1st bypass */
		scl_set_V_rec_table(index,0);	// bypass
		scl_set_V_scale_table(index,1);
		scl_set_V_div_table(index,0);
		index++;

		/* 2nd store into buffer */
		scl_set_V_rec_table(index,0);
		scl_set_V_scale_table(index,0);
		scl_set_V_div_table(index,0);
		index++;
		...
		for(i=0;i<(B-2);i++){
			scl_set_V_rec_table(index,0);
			scl_set_V_scale_table(index,3);
			scl_set_V_div_table(index,0x80);
			index++;
		}
		return;
	}
...
{
//	SCLMSG("scale RT(s %d,d %d,is H %d)\n",src,dst,horizontal);
...
//	SCLMSG("scale PP(s %d,d %d,is H %d)\n",src,dst,horizontal);
...
void sclw_set_framebuffer(vdo_framebuf_t *fb)
{
	...
	timing.read_cycle = vpp_set_base_clock(VPP_MOD_SCL,pixel_clock);
	scl_set_timing(&timing);
...
int scl_proc_scale(vdo_framebuf_t *src_fb,vdo_framebuf_t *dst_fb)
{
	...
	do {
		...
		vpp_proc_func(0,0,VPP_INT_SCL_VBIE,1);
		vpp_proc_func(0,0,VPP_INT_SCL_VBIE,1);
		...
		if( rd_cyc > 0xFD ){
			printk("[SCL] *E* scale error\n");
			ret = -1;
			break;
		}
		rd_cyc = rd_cyc + (0xFF - rd_cyc) / 2;
		if( vpp_check_dbg_level(VPP_DBGLVL_SCALE) ){
			printk("[SCL] scale retry, rcyc 0x%x\n",rd_cyc);
			printk("[SCL] TGERR %d,SCLW 0x%x\n",
	...
	vpp_proc_func(0,0,VPP_INT_SCL_VBIS,1);
	scl_set_tg_enable(VPP_FLAG_DISABLE);
