...
BOOL bHDMIMode = 1, bAudioEnable = 1 ;
#ifdef SUPPORT_DEGEN
MODE_ID ModeId ;
#endif
...
HDMITX_SetOutput()
{
    ...
    if( bHDMIMode )
    {
        ConfigAVIInfoFrame() ;
        DelayMS(100) ;            
     	EnableHDCP(TRUE) ;	// WMT Sam Shen
        
		if( bAudioEnable )
		{
			extern int cat6612_spdif_enable;	// WMT Sam Shen
		
		    SetNonPCMAudio(0) ; // For LPCM audio. If AC3 or other compressed audio, please set the parameter as 1
            EnableAudioOutput(VideoPixelClock*(pixelrep+1),INPUT_AUDIO_SAMPLE_FREQ, INPUT_AUDIO_CHANNEL, cat6612_spdif_enable);
            ConfigAudioInfoFrm() ;
		}
    }
    SetAVMute(FALSE) ;
...
#else
            ParseEDID() ;
            bAudioEnable = 1; // FALSE ; // default disable audio 
...
HDMITX_ChangeDisplayOption(HDMI_Video_Type OutputVideoTiming, HDMI_OutputColorMode OutputColorMode)
{
   //HDMI_Video_Type  t=HDMI_480i60_16x9;
    ...
    case HDMI_480i60:
        ...
        pixelrep = 1 ;		// 1:pixelclk double, 0:x156uyi	// [WMT Sam Shen]
//		pixelrep = 0;
        aspec = HDMI_4x3 ;
        Colorimetry = HDMI_ITU601 ;
        break ;
...
//      pixelrep = 1 ;		// 1:pixelclk double, 0:x156uyi	// [WMT Sam Shen]
    ...
    case HDMI_1080i50:
        ...
        Colorimetry = HDMI_ITU709 ;
        break ;
	case HDMI_1440x576i50:	// [WMT Sam Shen]
    #ifdef SUPPORT_DEGEN
	...
	bInputColorMode = bOutputColorMode;		// Add by WMT Sam Shen
