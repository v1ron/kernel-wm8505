...
_XDATA BYTE bOutputAudioMode = 0x40 ; // use full packet as default 
...
//    HDMITX_WriteI2C_Byte(REG_INT_CTRL,B_INTPOL_ACTL | B_INTPOL_ACTH) ; // depends on the interrupt type
	HDMITX_WriteI2C_Byte(REG_INT_CTRL,B_INTPOL_ACTL) ; // depends on the interrupt type
    ...
    HDMITX_WriteI2C_Byte(REG_INT_MASK1, 0xFE) ;		// 0xF8
    HDMITX_WriteI2C_Byte(REG_INT_MASK2, 0xFF) ;		// 0xF8
    HDMITX_WriteI2C_Byte(REG_INT_MASK3, 0xFF) ;		// 0xF7
	
	HDMITX_WriteI2C_Byte(REG_CLK_CTRL1,B_VDO_LATCH_EDGE);	// WMT Sam Shen
...
static _CODE struct SyncEmbeddedSetting SyncEmbTable[] = {
 ...
 //   VIC ,0x90, 0x91, 0x95, 0x96,0x97, 0xA0, 0xA1, 0xA2, 0xA3, 0x90_CCIR8,0x91_CCIR8    ,0x95_CCIR8,0x960_CCIR8   ,0x97_CCIR8  ,  PCLK    ,VFREQ},
	...
	{  32 ,0xE0, 0x37, 0x24, 0xA0,0x00, 0x04, 0x70, 0x0A, 0xD1, 0xE0      ,0x37          ,0x23      ,0x9F          ,0x00            , 27000000,60},    
    {  33 ,0x60, 0x2E, 0x16, 0x94,0x00, 0x02, 0x50, 0x3A, 0xD1, 0x50      ,0x37          ,0x15      ,0x93          ,0x00            , 27000000,50},
...
//     }
    ...
    FireAFE() ;
    DelayMS(100) ;
//  HDMITX_AndReg_Byte(REG_INT_MASK3, ~B_VIDSTABLE_MASK) ;	// WMT Sam Shen
...
SetInputMode(BYTE InputColorMode, BYTE InputSignalType)
{
    ...
    if( InputSignalType & T_MODE_INDDR )
    {
		...
		HDMITX_WriteI2C_Byte(REG_CLK_CTRL1,0);	// WMT Sam Shen		
	}
...
SetAudioFormat(BYTE NumChannel, BYTE AudioEnable, ULONG SampleFreq, BYTE AudSWL, BYTE AudioCatCode)
{
   ...
   if( AudioEnable & B_AUD_SPDIF )
	...
	bOutputAudioMode |= B_AUDFMT_32BIT_I2S;	// WMT Sam Shen
    AudioEnable |= B_AUD_EN_I2S0 ;	// WMT Sam Shen
    ...
    HDMITX_WriteI2C_Byte(REG_AUDIO_CTRL0, AudioEnable&0xF1) ; // disable audio channel before audio enabled.	// WMT Sam Shen
...
HDCP_CheckSHA(BYTE pM0[],USHORT BStatus,BYTE pKSVList[],int cDownStream,BYTE Vr[])
{
    ...
    for(i = 0 ; i < 64 ; i++)
	{
		if(i % 16 == 0) ErrorF("SHA[]: ") ;
		ErrorF(" %02X",SHABuff[i]) ;
		if((i%16)==15) ErrorF("\n") ;
	}
