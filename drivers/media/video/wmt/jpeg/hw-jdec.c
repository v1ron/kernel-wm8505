...
static int ioctl_decode_proc(jdec_drvinfo_t *drvinfo, jdec_input_t *arg)
{
    ...
    prd_virt = (struct prdt_struct *)drvinfo->prd_virt;
    /*--------------------------------------------------------------------------
    ...
    if(drvinfo->_multi_seg_en) {
        ...
        if(drvinfo->_remain_byte == drvinfo->attr.hdr.filesize) {
        ...
        drvinfo->_remain_byte -= arg->src_size;
        if(drvinfo->_remain_byte == 0) {
            drvinfo->_segment |= JDEC_SEG_LAST;  /* last segment */
        }    
        DBG_MSG("_segment: 0x%x\n", drvinfo->_segment );
        
        if(drvinfo->_prd_offset) { /* reserved bytes in PRD table */
            prd_virt = (struct prdt_struct *)((char *)drvinfo->prd_virt + drvinfo->_prd_offset);
            memset(prd_virt, 0, drvinfo->_prd_offset);
        }
    }
    memcpy(&drvinfo->arg_in, arg, sizeof(jdec_input_t));
    ...
    --------------------------------------------------------------------------*/
    if(user_to_prdt(arg->src_addr, arg->src_size, prd_virt, 
...
static int copy_frame(jdec_drvinfo_t *drvinfo, jdec_frameinfo_t *frame_info)
{
    ...
    down_interruptible(&jdec_decoding_sema); 
...
static int jdec_open(struct inode *inode, struct file *filp)
{
    ...
    drvinfo->_segment  = JDEC_SEG_NONE;
    drvinfo->_is_mjpeg = 0;
    drvinfo->_prd_offset = 8;  // For WM3426 only
...
MODULE_DESCRIPTION("WM8510 JPEG decode device driver");
MODULE_LICENSE("GPL");
