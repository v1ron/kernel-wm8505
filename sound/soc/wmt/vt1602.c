 ...
 */
static inline u16  vt1602_read_reg_cache(unsigned int reg)
{
...
u16  vt1602_read_reg(unsigned int index ) {
	
	return vt1602_read_reg_cache(index);
}

EXPORT_SYMBOL_GPL(vt1602_read_reg);
/*
 * write wm8750 register cache
 */
static inline void vt1602_write_reg_cache( unsigned int reg, u16   value)
{
	if (reg > VT1602_CACHE_REGNUM)
		return;
	reg_cache[reg] = value;
...
int vt1602_write_reg(unsigned int index , u16 value) {
	char* buf ;
	struct i2c_msg msg[1];
	vt1602_write_reg_cache (index, value);
...
EXPORT_SYMBOL_GPL(vt1602_write_reg);
int vt1602_reset () {
...
int vt1602_init(void)
{
	unsigned int i = 0;
        if(reg_cache == NULL)
        {
                ...
                reg_cache = kmemdup(vt1602_reg, sizeof(vt1602_reg), GFP_KERNEL);
                if (reg_cache == NULL)
                        return -ENOMEM;
        }
